...
export default function Header({ query, setQuery, onSelect }: Props) {
  ...
  const suggestions = useMemo(() => {
    ...
    const arts = ARTICLES.filter((a) => re.test(a.title) || re.test(a.summary)).slice(0, 6);
    const comps = COMPANIES.filter((c) => re.test(c.name) || re.test(c.name_en)).slice(0, 4);
    return { arts, comps };
  }, [query]);
  ...
  return (
    <header className="header">
            ...
            <div className="cmd-hint">
              ...
              {query && (
                <>
                  ...
                  {(suggestions as { arts?: typeof ARTICLES }).arts?.length ? (
                    <div className="ch-section">
      ...
      <div className="header-right">
        <button className="hbtn"><span className="live-dot" /><span style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5 }}>LIVE · 6h sync</span></button>
