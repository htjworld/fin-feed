...
type Props = { onOpen: (c: Collection) => void; query?: string };
    ...
    <section className="collections-section">
      <div className="coll-section-head">
        <div>
          ...
          <h2 className="coll-section-title">금융 IT의 <span className="ital">클래식</span>만 모았다</h2>
          <p className="coll-section-desc">매주 변하는 피드 너머, 직군·도메인별로 한 번은 꼭 읽어야 할 글을 묶었습니다. 큐레이션은 사람이.</p>
        </div>
        <button className="tbtn">전체보기 <Ic.chevron /></button>
      </div>
      <div className="coll-grid">
        {matchingColls.map((c) => <CollectionCard key={c.id} collection={c} onOpen={onOpen} />)}
      </div>
    </section>
  );
}
