...
import {
  ARTICLES, COLLECTIONS, COMPANIES,
  COMPANY_BY_ID, SECTOR_BY_ID,
} from '@/data';
...
export default function FinFeedApp() {
  ...
  const filtered = useMemo(() => {
    ...
    // On home (no filters/search), exclude the pinned hero article from the grid
    if (showHero) {
    ...
    if (filters.sector !== 'all') {
      arr = arr.filter((a) => COMPANY_BY_ID[a.company].sector === filters.sector);
    }
    if (filters.companies.length > 0) {
      arr = arr.filter((a) => filters.companies.includes(a.company));
    }
    if (filters.categories.length > 0) {
      arr = arr.filter((a) => a.tags.some((t) => filters.categories.includes(t)));
    }
    ...
    if (query) {
      ...
      const matchingColls = COLLECTIONS.filter(
        (c) => c.title.toLowerCase().includes(q) || c.desc.toLowerCase().includes(q) || c.subtitle.toLowerCase().includes(q)
      );
      ...
      arr = arr.filter(
        (a) =>
          ...
          COMPANY_BY_ID[a.company].name.toLowerCase().includes(q) ||
          COMPANY_BY_ID[a.company].name_en.toLowerCase().includes(q) ||
          idsFromColls.has(a.id)
      );
    }
        ...
        <div className="main-inner">
          <div className="titlebar">
            <div className="title-block">
              ...
              <div className="page-meta" style={{ marginTop: 6 }}>
                ...
                <span className="sep">·</span>
                <span>{[...new Set(filtered.map((a) => a.company))].length}개 회사</span>
                <span className="sep">·</span>
                {activeCollection ? (
            ...
            <div className="coll-header" style={{ '--coll-accent': activeCollection.accent } as React.CSSProperties}>
              <div>
                ...
                <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#BBCBC5', alignSelf: 'center', letterSpacing: '.06em' }}>
                    EDITOR&apos;S PICK · 사람이 직접 선별한 클래식
                  </span>
                </div>
              </div>
              <div className="coll-header-stats">
                ...
                <div className="coll-header-stat">
                  <span className="n">{[...new Set(filtered.map((a) => COMPANY_BY_ID[a.company].sector))].length}</span>
                  <span className="l">SECTORS</span>
                </div>
              </div>
            </div>
          )}
              ...
              <div className="hero-right">
                <div className="hero-thumb">
                  <div className="hero-thumb-frame">
                    <Thumbnail article={featured} company={fCompany} sector={fSector} />
                  </div>
                ...
                <div className="hero-stats">
                  ...
                  <div className="hero-stat">
                    <span className="num">+12<span className="unit">today</span></span>
                    <span className="lab">New this 24h</span>
                  </div>
                  <div className="hero-stat">
          ...
          {filters.sector !== 'all' && !isSearch && !activeCollection && (
            <div className="sector-banner">
              <div className="sb-mark" style={{ background: SECTOR_BY_ID[filters.sector].accent, color: '#fff' }}>
                {SECTOR_BY_ID[filters.sector].label[0]}
              </div>
              ...
              <div className="sb-stats">
                ...
                <div className="sb-stat">
                  <span className="num" style={{ color: 'var(--accent)' }}>+{Math.floor(filtered.length / 8)}</span>
            ...
            <>
              {isSearch && (
                <div className="section-head">
                  ...
                  <span className="sub">{filtered.length} HIT{filtered.length === 1 ? '' : 'S'}</span>
                </div>
              )}
              <div className={`grid ${view === 'list' ? 'list' : density === 'dense' ? 'dense' : ''}`}>
                {filtered.map((a) => (
                  <ArticleCard
                    ...
                    highlightTags={filters.categories}
                  />
                ))}
              </div>
            </>
